...
def cwloss(output, target,confidence=50, num_classes=10):
    target = target.data
    target_onehot = torch.zeros(target.size() + (num_classes,), device=target.device)
...
def pgd(model, data, target, epsilon, step_size, num_steps,loss_fn,category,rand_init):
    model.eval()
    if category == "trades":
        x_adv = data.detach() + 0.001 * torch.randn(data.shape, device=data.device).detach() if rand_init else data.detach()

    if category == "Madry":
        noise = torch.from_numpy(np.random.uniform(-epsilon, epsilon, data.shape)).float().to(data.device)
        x_adv = data.detach() + noise if rand_init else data.detach()
