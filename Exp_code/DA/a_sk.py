...
parser.add_argument('--rs',        default=[283,     283,     283,     283,     283,     283,     283,     283,     283,     283],  help = 'Random seed')
...
Y = torch.load('imageneta_Fea.pt')
X = torch.load('imagenetsk_Fea.pt')
...
for dd in range(len(args.N1)):
    ...
    for kk in range(args.n_exp):
        ...
        np.savetxt('../../Results/DA/'+'a_sk_tp_'+str(args.N1), Results.reshape(Results.shape[0],-1), fmt='%.3f')
        # np.savetxt('../../Results/DA/'+'a_sk_pv_'+str(args.N1), Results_P.reshape(Results_P.shape[0],-1), fmt='%.10f')
    ...
    np.savetxt('../../Results/DA/'+'a_sk_T_'+str(args.N1), Final_results.reshape(Final_results.shape[0],-1), fmt='%.3f')
    # np.savetxt('../../Results/DA/'+'a_sk_P_'+str(args.N1), Final_results_P.reshape(Final_results_P.shape[0],-1), fmt='%.10f')
