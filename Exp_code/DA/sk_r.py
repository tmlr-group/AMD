...
# parameters of experimental setting
parser.add_argument('--N1',        default=[200,     300,     400,     500,     600,     700,     800,     900,     1000,    1100],    help = 'Size of each sample in optimization')
parser.add_argument('--rs',        default=[183,     183,     183,     184,     183,     192,     193,     183,     183,     183],  help = 'Random seed')
...
Y = torch.load('imagenetsk_Fea.pt')
X = torch.load('imagenetr_Fea.pt')
...
for dd in range(len(args.N1)):
    ...
    for kk in range(args.n_exp):
        ...
        np.savetxt('../../Results/DA/'+'sk_r_tp_'+str(args.N1), Results.reshape(Results.shape[0],-1), fmt='%.3f')
        # np.savetxt('../../Results/DA/'+'sk_r_pv_'+str(args.N1), Results_P.reshape(Results_P.shape[0],-1), fmt='%.10f')
    ...
    np.savetxt('../../Results/DA/'+'sk_r_T_'+str(args.N1), Final_results.reshape(Final_results.shape[0],-1), fmt='%.3f')
    # np.savetxt('../../Results/DA/'+'sk_r_P_'+str(args.N1), Final_results_P.reshape(Final_results_P.shape[0],-1), fmt='%.10f')
